"""Safe conversion of Hermes tool lifecycle events into spoken progress."""
...
class HermesToolProgress:
    """Structured lifecycle event emitted by Hermes chat completions."""
    ...
    def from_json(cls, data: str) -> "HermesToolProgress | None":
        ...
        if not tool or status not in {"running", "completed", "failed"}:
            return None
        return cls(
            tool=tool,
            status=status,
    ...
    def technical_log(self) -> dict[str, str]:
        """Return lifecycle fields with potentially sensitive previews redacted."""
...
_CATEGORIES = (
    ...
    _ProgressCategory(
        "music_service",
        "正在连接音乐服务",
        ("music_login", "music_connect", "spotify_auth", "netease_auth"),
    ),
    _ProgressCategory(
        "music",
        "正在帮你找这首歌",
        ("music", "song", "playlist", "spotify", "netease", "youtube_music"),
        ("歌曲", "这首歌", "音乐", "歌单", "播放"),
    ),
    _ProgressCategory(
        "home",
        "正在查看家里的设备状态",
        ("home_assistant", "homeassistant", "smart_home", "hass", "iot", "device_state"),
    ...
    _ProgressCategory(
        "research",
        "正在查最新资料",
        ("web_search", "web_extract", "browser", "search", "fetch", "wikipedia", "news"),
        ("最新", "查一下", "搜索", "资料", "新闻"),
    ),
    _ProgressCategory(
        "memory",
...
class HermesProgressNarrator:
    """Classify, deduplicate and rate-limit Hermes progress semantics.

    Tool labels remain available for technical logs but are deliberately not
    used as spoken text because they may contain URLs, arguments or commands.
    """
    ...
    def observe(self, event: HermesToolProgress):
        category = self._classify_tool(event.tool) or self._classify_task()
        if category:
            self._latest_category = category

        call_key = event.tool_call_id or f"{event.tool}:{event.status}"
    ...
    def next_message(self) -> tuple[str, str] | None:
        """Return a predefined safe message and semantic de-duplication key."""
    ...
    def _classify_tool(self, tool: Any) -> str | None:
        ...
        if not normalized:
            return None
        # Connection/auth categories must win over the broader music match.
        for category in _CATEGORIES:
            if any(term in normalized for term in category.tool_terms):
                return category.name
        return None

    def _classify_task(self) -> str | None:
        for category in _CATEGORIES:
            if any(term in self._user_text for term in category.task_terms):
                return category.name
        return None
