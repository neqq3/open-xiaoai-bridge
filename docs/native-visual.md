...
`dev/hermes-native-visual` 合并独立 Hermes PR 和灯效分支用于 NAS 实测。Hermes 保持原来的
SSE、工具进度、Session/Profile 和分句语音队列；每条进度/回答进入相同原厂 speech 路径，
工具进度说完且最终正文尚未到达时恢复等待蓝灯。原厂抢占时流式清理也不追加全局暂停。

`Dockerfile.native-visual` 用于基于已验证依赖镜像构建新的集成镜像；须先核对基础镜像源码的
Rust 与依赖锁文件一致，构建时传入不可变 `BASE_IMAGE` 及 `SOURCE_REVISION`。
它不是跨任意历史镜像更新依赖的通用 Dockerfile。
