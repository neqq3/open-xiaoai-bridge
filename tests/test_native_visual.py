...
class NativeVisualTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_normal_visual_expiry_does_not_abort_long_agent_request(self):
        expired = types.SimpleNamespace(exit_code=25, stdout="visual_result=25 native_takeover=0\n")
        self.session.task = asyncio.create_task(asyncio.sleep(0, result=expired))
        await self.session.clear()
        self.assertFalse(self.session.preempted.is_set())
